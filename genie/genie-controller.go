...
type SetStatus func(current.Result, string, interface{}) interface{}
...
func AddPodNetwork(cniArgs utils.CNIArgs, conf utils.GenieConf) (types.Result, error) {
	...
	result, status, err := addNetwork(plugins, cniArgs, setGenieStatus)
	if err != nil {
		return nil, err
	}
...
func parseResult(setStatus SetStatus, ch chan sendCh) (types.Result, interface{}) {
	...
	for r := range ch {
		...
		status = setStatus(*currentResult, r.ifName, status)
	}
...
func addNetwork(pluginElements []*utils.PluginInfo, cniArgs utils.CNIArgs, setStatus SetStatus) (types.Result, interface{}, error) {
	...
	fmt.Fprintf(os.Stderr, "CNI Genie returning from addNetwork: endResult: %v;  status: %v\n", endResult, status)
...
func deleteNetwork(pluginElements []*utils.PluginInfo, cniArgs utils.CNIArgs) error {
	...
	for i := len(pluginElements) - 1; i >= 0; i-- {
		...
		if err != nil {
			cnierr = err
			fmt.Fprintf(os.Stderr, "CNI Genie Error while deleting network (%s) %v", pluginElement.PluginName, err)
			continue
		}
	}
...
func loadPluginInfoList(files, plugins []string) ([]*utils.PluginInfo, error) {
	...
	for i := range plugins {
		if pluginMap[strings.TrimSpace(plugins[i])] == nil {
			pluginMap[strings.TrimSpace(plugins[i])] = map[bool][]int{false: {i + 1}}
		} else {
			pluginMap[strings.TrimSpace(plugins[i])][false] = append(pluginMap[strings.TrimSpace(plugins[i])][false], i+1)
		}
	}
	...
	for _, file := range files {
		...
		pluginName := file[strings.LastIndex(file, "/")+1 : strings.LastIndex(file, ".")]
	...
	fmt.Fprintf(os.Stderr, "CNI Genie final plugin list: %+v\n", pluginInfoList)
...
func parseCNIAnnotations(annot map[string]string, client *kubernetes.Clientset, k8sArgs utils.K8sArgs, conf utils.GenieConf) ([]*utils.PluginInfo, error) {
	...
	fmt.Fprintf(os.Stderr, "CNI Genie total number of finalPluginInfos= %v\n", len(finalPluginInfos))
